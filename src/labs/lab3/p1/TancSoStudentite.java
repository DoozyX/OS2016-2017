...
/**
 * Dancing with the Students Problem 1 (0 / 0)
 ...
 * <p>
 * After they change, the contestants wait for a random partner from the opposite gender, after which they exit the changing room and enter the dancing room.
 * The dancing room can have a maximum of three pairs dancing at the same time (in parallel).
 * After they finish dancing, the pairs exit the dancing room leaving space for a new pair to enter and start dancing.
 * <p>
 * In the starter code, you have the definition of two classes, Masko (Male) and Zensko (Female), which represent contestants from the corresponding gender.
 * Each of the classes has multiple running instances, and each instance can dance only once.
 * <p>
 ...
 * <p>
 * Your task is to implement the methods Masko.ucestvo(), Zensko.ucestvo() and init(). In your implementation, you can not add try-catch blocks in them.
 * You should define the necessary semaphores, global variables and state variables.
 * <p>
...
public class TancSoStudentite {
	...
	static int PERSONS_PER_ROOM = 10;
	public static final int DANCING = 3;
	Semaphore meleRoom;
	Semaphore femaleRoom;
	Semaphore femaleReadyToDance;
	Semaphore dancing;

	public void init() {
		//TODO: da se implementira
		meleRoom = new Semaphore(PERSONS_PER_ROOM);
		femaleRoom = new Semaphore(PERSONS_PER_ROOM);
		femaleReadyToDance = new Semaphore(0);
		dancing = new Semaphore(DANCING);
	}
