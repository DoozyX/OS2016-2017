...
/**
 * Tribe Dinner Problem 3
 * A tribe is eating a dinner prepared in a large kettle which holds a limited number of portions.
 * Each tribe member takes a portion on his own, if there is food in the kettle.
 * If the kettle is empty, the cooking chef is called to prepare a new kettle full of food. Only one tribe member can check if the kettle is empty at a given moment.
 * However, there are three places around the kettle, meaning three tribe members can take food in parallel. The number of member eating concurrently is not limited.
 * <p>
 * The kettle is empty initially.
 * <p>
 * Your task is to synchronize the given scenario.
 * <p>
 * In the starter code, you have a definition of the TribeMember class, which represents the tribe members and their behavior.
 * There are multiple instances of TribeMember in which the execute() method is called multiple times.
 * <p>
 * In your implementation you can use the following methods from state:
 * <p>
 * state.isPotEmpty()
 * Used to check if there is still food in the kettle. It manipulates with a global variable and should be treated as a critical region.
 * If more than one tribe member checks at the same time, the call will throw an exception.
 * state.fillPlate()
 * Represents taking food from the kettle.
 * If the kettle is empty, or more then 3 tribe member all it in parallel, it will throw an exception
 * state.eat()
 * Represents the consuming of food by the tribe member.
 * state.cook()
 * Represents the cooking done by the cooking chef.
 * If the kettle is not empty, it will throw an exception.
 * These methods are used for validation the scenario and cannot be changed. They need to be called.
 * <p>
 * Your task is to implement the methods TribeMember.execute() and init().
...
public class TribeDinner {
	...
	public static void init(int numBarbers) {
		plates = new Semaphore(CAN_EAT_PARALEL);
